...
This guide covers how to publish the `mystereogram` package to the Python Package Index (PyPI). The process is automated using a `Makefile` for simplicity and consistency.
...
- [ ] Verify package published at [https://pypi.org/project/mystereogram/](https://pypi.org/project/mystereogram/)
...
2. **Generate an API token**:
   ...
   - Create a new token (scope it to the `mystereogram` project if desired)
...
[project]
name = "mystereogram"
version = "0.2.0" # Example: changed from 0.1.0
# ...
```
...
**Note**: Replace `0.2.0` with your actual version number. Tags should follow the `v{version}` format.
